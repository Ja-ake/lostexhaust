    ...
    public static List<CatlinPerson> getPeople(String... userId) throws IOException {
        ...
        if (Arrays.stream(userId).allMatch(matchesId::test)) {
            ...
            peopleArray.forEach(k -> {
                ...
                ((JSONArray) o.get("contacts")).forEach(
                        x -> contacts.add(new Contact(((JSONObject) x).getString("contact_type") + " " + ((JSONObject) x).getString("contact_mode"), ((JSONObject) x).getString("contact_value"))));

                List<Relationship> relationships = new ArrayList<>();
                ((JSONArray) o.get("relationships")).forEach(x -> relationships.add(new Relationship(new CatlinUserContext(((JSONObject) x).getString("relation_id")),
                        ((JSONObject) x).getString("relationship"), ((JSONObject) x).getString("relation_firstname"), ((JSONObject) x).getString("relation_lastname"))));

                people.add(new CatlinPerson(succOrNull(() -> o.getString("eeid")), succOrNull(() -> o.getString("firstname")), succOrNull(() -> o.getString("lastname")),
                        succOrNull(() -> o.getString("gradelevel")), succOrNull(() -> o.getString("classyear")), succOrNull(() -> o.getString("affiliation")), contacts, relationships));
            });
